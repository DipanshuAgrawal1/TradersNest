...
class VendorProductVariantItemController extends Controller
{
    public function index(VendorProductVariantItemDataTable $dataTables, $productId, $variantId)
    {
        ...
        return $dataTables->render('vendor.product.variant-item.index', compact('product', 'variant'));
    }
    ...
    {
        $varientItem = ProductVariantItem::findOrFail($request->id);
        $varientItem->status = $request->status == 'true' ? 1 : 0;
        $varientItem->save();
